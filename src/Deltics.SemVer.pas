...
implementation
  ...
  var
    a, b: TSemVer;
  begin
    a := TSemVer.Create(aSemVer);
    b := TSemVer.Create(aOtherSemVer);
    try
      result := TSemVer.Compare(a, b);
...
{ TSemVer }
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (result = 0) then
            result := Abs(High(identsB) - High(identsA));
        end;
      end;
    end;
  end;
  ...
  procedure TSemVer.Parse(const aVersion: String);
  const
  ...
  begin
    ...
    try
      if NOT TStringTemplate.Match([TEMPLATE_VersionIdentifiersMetadata,
        ...
        raise EArgumentException.CreateFmt('''%s'' is not a valid SemVer version string', [aVersion]);

      fMajorVersion := STR.AsInteger(vars.Values['major']);
      fMinorVersion := STR.AsInteger(vars.Values['minor']);
      fPatch        := STR.AsInteger(vars.Values['patch']);
...
{ TIdentifierList }
  ...
  begin
    if STR.IsInteger(fItems[aIndex]) then
      result := itInteger
    else
      result := itString;
  end;
...
{ TSemVerReference }
  ...
  begin
    ...
    try
      ...
      if major = '*' then fMajorVersion := ANY_VERSION else fMajorVersion := STR.AsInteger(major, ERROR);
      if minor = '*' then fMinorVersion := ANY_VERSION else fMinorVersion := STR.AsInteger(minor, ERROR);
      if patch = '*' then fPatch        := ANY_VERSION else fPatch        := STR.AsInteger(patch, ERROR);
