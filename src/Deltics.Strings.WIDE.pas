...
interface
  ...
  type
    WIDEFn = class
    private
      class function AddressOfByte(aBase: Pointer; aByteIndex: Integer): PWIDEChar; overload; {$ifdef INLINE_METHODS} inline; {$endif}
      class function AddressOfIndex(aBase: PWIDEChar; aIndex: Integer): PWIDEChar; overload; {$ifdef INLINE_METHODS} inline; {$endif}
      class function AddressOfIndex(var aString: UnicodeString; aIndex: Integer): PWIDEChar; overload; {$ifdef INLINE_METHODS} inline; {$endif}
      class procedure FastCopy(const aString: UnicodeString; aDest: PWIDEChar); overload; {$ifdef INLINE_METHODS} inline; {$endif}
      class procedure FastCopy(const aString: UnicodeString; aDest: PWIDEChar; aLen: Integer); overload; {$ifdef INLINE_METHODS} inline; {$endif}
      class procedure FastCopy(const aString: UnicodeString; var aDest: UnicodeString; aDestIndex: Integer); overload; {$ifdef INLINE_METHODS} inline; {$endif}
      class procedure FastCopy(const aString: UnicodeString; var aDest: UnicodeString; aDestIndex: Integer; aLen: Integer); overload; {$ifdef INLINE_METHODS} inline; {$endif}
      class procedure FastMove(var aString: UnicodeString; aFromIndex, aToIndex, aCount: Integer); overload; {$ifdef INLINE_METHODS} inline; {$endif}
...
//      class function Replace(aScope: TStringScope; const aString, aFindStr, aReplaceStr: UnicodeString; var aCount: Integer; aCaseMode: TCaseSensitivity): UnicodeString; overload;
    ...
    public
      class function Parse: WIDEParserClass; {$ifdef INLINE_METHODS} inline; {$endif}
      ...
      class function FromANSI(aBuffer: PANSIChar; aMaxLen: Integer): UnicodeString; overload;
      ...
      class function FromBuffer(aBuffer: PWIDEChar; aLen: Integer = -1): UnicodeString;
...
      class function Split(const aString: UnicodeString; const aChar: ANSIChar; var aParts: TWIDEStringArray): Boolean; overload;
      class function Split(const aString: UnicodeString; const aChar: WIDEChar; var aParts: TWIDEStringArray): Boolean; overload;
      class function Split(const aString, aDelim: UnicodeString; var aParts: TWIDEStringArray): Boolean; overload;
...
{ WIDEFn ------------------------------------------------------------------------------------------ }
  ...
  var
    len: Integer;
  begin
    Contract.Assigned(aBuffer);
    Contract.Minimum(aMaxChars, 0);
...
*)
  ...
  var
    ...
    subLen: Integer;
  begin
    Contract.NotEmpty(aSubString, subLen);
  ...
  begin
    ...
    result := (len <= Length(aString))
          and (CompareStringW(LOCALE_USER_DEFAULT, NORM_IGNORECASE,
                              PWIDEChar(aString) + System.Length(aString) - len, len,
                              PWIDEChar(aSubstring), len) = CSTR_EQUAL);
  end;
  ...
  begin
    ...
    try
      if  (aPos < 1)
       or (NOT HasLength(aString, strLen))
       or (NOT HasLength(aSubstring, subLen)) then
        EXIT;
  ...
  begin
    ...
    subLen  := System.Length(aSubstring);
  ...
  begin
    Contract.NotEmpty(aDelim, delimLen);
  ...
  class procedure WIDEFn.Delete(var aString: UnicodeString;
                                    aIndex: Integer);
  begin
    Contract.ValidIndex(aString, aIndex);
  ...
  begin
    Contract.ValidIndex(aString, aIndex);
    Contract.ValidIndex(aString, aEndIndex);
  ...
  begin
    ...
    Contract.ValidIndex(aString, aIndex,               'Invalid index for extract');
    Contract.ValidIndex(aString, aIndex + aLength - 1, 'Extract exceeds length of string');
  ...
  var
    p, delimLen: Integer;
  begin
    Contract.NotEmpty(aDelimiter, delimLen);
