...
interface
  ...
  type
    ...
    ['{EC71F82C-C696-4421-96AD-9B6D1318AFFD}']
      ...
      function Peek(var aChar: WIDEChar): Boolean; overload;
      function ReadChar: WideChar; overload;
...
implementation
  ...
  type
    ...
    public
      constructor Create(aStream: TStream; aDefaultEncoding: TEncoding; aBufferKB: Word); overload;
      constructor CreateAsOwnerOf(aStream: TStream; aDefaultEncoding: TEncoding; aBufferKB: Word); overload;
...
{ TUnicodeStringReader }
  ...
  function TUnicodeStringReader.Peek: WIDEChar;
  begin
    ASSERT(fPos < Length(fString));
    result := fString[fPos + 1];
  end;


  function TUnicodeStringReader.Peek(var aChar: WIDEChar): Boolean;
  begin
    result := fPos < Length(fString);
    if result then
      aChar := fString[fPos + 1]
    else
      aChar := #0;
  end;


  function TUnicodeStringReader.ReadChar: WIDEChar;
  begin
    ASSERT(fPos < Length(fString));
...
{ TUnicodeWideCharReader }
  ...
  function TUnicodeWideCharReader.Peek: WIDEChar;
  begin
    ASSERT(fBufferPos < fBufferLength);
    result := fBuffer[fBufferPos + 1];
  end;


  function TUnicodeWideCharReader.Peek(var aChar: WIDEChar): Boolean;
  begin
    result := fBufferPos < fBufferLength;
    if result then
      aChar := fBuffer[fBufferPos + 1]
    else
      aChar := #0;
  end;


  function TUnicodeWideCharReader.ReadChar: WIDEChar;
  begin
    ASSERT(fBufferPos < fBufferLength);
  ...
  constructor TUnicodeStreamReader.Create(aStream: TStream;
                                          aDefaultEncoding: TEncoding;
                                          aBufferKB: Word);
  begin
    DetectEncoding(aStream, aDefaultEncoding);
  ...
  constructor TUnicodeStreamReader.CreateAsOwnerOf(aStream: TStream;
                                                   aDefaultEncoding: TEncoding;
                                                   aBufferKB: Word);
  begin
    Create(aStream, aDefaultEncoding, aBufferKB);
  ...
  function TUnicodeStreamReader.Peek: WIDEChar;
  begin
  ...
  function TUnicodeStreamReader.Peek(var aChar: WIDEChar): Boolean;
  begin
    if (fBufferPos = fBufferLength) then
      FillBuffer;
  ...
  function TUnicodeStreamReader.ReadChar: WIDEChar;
  begin
    if (fBufferPos = fBufferLength) then
      FillBuffer;
